...
generate_ccov(){
    ...
    run_command "lcov --remove $BUILD_DIR/coverage.info ' */install/include/* */usr/include/* */include/weston/* */build_ivi_extension/* \
                                                          */wayland-ivi-extension/unittest/* */bitmap.c */writepng.c */src/*.h ' \
                                                          -o $BUILD_DIR/coverage.info --rc lcov_branch_coverage=1"
...
if [ -z "$1" ]; then
    ...
    BUILD_DIR=$UNITTEST_DIR/../build_ivi_extension/
    print_information "Default directory: ${BUILD_DIR}" # NOTE! default build folder is build_ivi_extension
else
    BUILD_DIR=$1
    print_information "Current directory: ${BUILD_DIR}"
fi
...
if [ -d "$BUILD_DIR" ]; then
    ...
    print_error "'$BUILD_DIR' NOT found."
fi
