...
TEST_F(InputControllerTest, keyboard_grab_key_success)
{
    ...
    struct weston_surface *l_surf[1];
    l_surf[0] = (struct weston_surface *)malloc(sizeof(struct weston_surface));
    l_surf[0]->resource = (struct wl_resource *)malloc(sizeof(struct wl_resource));
    SET_RETURN_SEQ(surface_get_weston_surface, &l_surf[0], 1);
...
TEST_F(InputControllerTest, pointer_grab_focus_surfNotEnabledAndNullFocus)
{
    ...
    struct weston_surface *l_surf[1];
    l_surf[0] = (struct weston_surface *)0xFFFFFFFF;
    SET_RETURN_SEQ(surface_get_weston_surface, &l_surf[0], 1);
...
TEST_F(InputControllerTest, pointer_grab_focus_success)
{
    ...
    struct ivi_layout_surface *l_layout_surf[1];
    l_layout_surf[0] = &mp_layoutSurface[0];
    SET_RETURN_SEQ(get_surface, &l_layout_surf[0], 1);

    struct weston_surface *l_surf[1];
    l_surf[0] = (struct weston_surface *)malloc(sizeof(struct weston_surface));
    custom_wl_list_init(&l_surf[0]->views);
    SET_RETURN_SEQ(surface_get_weston_surface, &l_surf[0], 1);
    SET_RETURN_SEQ(weston_surface_get_main_surface, &l_surf[0], 1);
...
{
    struct input_context *l_input[1];
    l_input[0] = mp_ctxInput;
    SET_RETURN_SEQ(wl_resource_get_user_data, (void**)&l_input[0], 1);
...
TEST_F(InputControllerTest, input_set_input_focus_wrongDevice)
{
    ...
    struct ivi_layout_surface *l_surface[1];
    l_surface[0] = &mp_layoutSurface[0];
    SET_RETURN_SEQ(get_surface_from_id, &l_surface[0], 1);
...
TEST_F(InputControllerTest, input_set_input_focus_toKeyBoardWithOneKeyBoardAndNotEnabled)
{
    ...
    struct weston_keyboard *l_keyboard[1];
    l_keyboard[0] = (struct weston_keyboard *)0xFFFFFFFF;
    SET_RETURN_SEQ(weston_seat_get_keyboard, &l_keyboard[0], 1);
...
TEST_F(InputControllerTest, input_set_input_focus_toKeyBoardWithOneKeyBoardAndEnabled)
{
    ...
    struct weston_surface *l_weston_surfaces[1];
    l_weston_surfaces[0] = (struct weston_surface *)malloc(sizeof(struct weston_surface));
    SET_RETURN_SEQ(surface_get_weston_surface, &l_weston_surfaces[0], 1);

    int l_num[1] = {WL_KEYBOARD_REPEAT_INFO_SINCE_VERSION};
    SET_RETURN_SEQ(wl_resource_get_version, &l_num[0], 1);
...
TEST_F(InputControllerTest, input_set_input_focus_toPointerWithOnePointerAndNotEnabled)
{
    ...
    struct weston_pointer *l_ptr[1];
    l_ptr[0] = (struct weston_pointer *)0xFFFFFFFF;
    SET_RETURN_SEQ(weston_seat_get_pointer, &l_ptr[0], 1);
